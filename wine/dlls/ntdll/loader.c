...
#define HOOK_CODE_SIZE 1024
#define NAME_OFFSET (HOOK_CODE_SIZE-100)
#define PARAM_OFFSET (HOOK_CODE_SIZE-50)
#define ORIGFUNC_ADDR_OFFSET (HOOK_CODE_SIZE-80)
#define PARSE_ENTRY_ADDR_OFFSET (HOOK_CODE_SIZE-4)
#define PARSE_EXIT_ADDR_OFFSET (HOOK_CODE_SIZE-8)

#define IS_INTARG(x)       (((ULONG_PTR)(x) >> 16) == 0)

static PeekPeek *peekpeek = NULL;
...
static unsigned int get_ptr_size(void)
{
	return 4;
}

char get_arg_type(char* token)
{
	char type;
	int arg;
	...
	switch (arg)
    {
        case ARG_STR: type ='s'; break;
        case ARG_WSTR: type = 'w'; break;
        case ARG_FLOAT: type = 'f'; break;
        case ARG_DOUBLE: type = 'd'; break;
        case ARG_INT64:
        case ARG_INT128:
            if (get_ptr_size() == (unsigned int)4)
            {
                type = (arg == ARG_INT64) ? 'j' : 'k';
                break;
            }
            /* fall through */
        case ARG_LONG:
...
static void print_string_a( INT_PTR ptr )
{
    if (!IS_INTARG( ptr )) 
    {
	    //printf( "%08lx %s", ptr, debugstr_a( (char *)ptr ));
	    FIXME( "%08lx %s", ptr, debugstr_a( (char *)ptr ));
    }
    else 
    {
	    //printf( "%08lx", ptr );
...
static void print_string_w( INT_PTR ptr )
{
    if (!IS_INTARG( ptr )) 
    {
	    //printf( "%08lx %s", ptr, debugstr_w( (WCHAR *)ptr ));
	    FIXME( "%08lx %s", ptr, debugstr_w( (WCHAR *)ptr ));
    }
    else 
    {
	    //printf( "%08lx", ptr );
...
void  WINAPI parse_entry( char* name, char* params,const DWORD *stack, unsigned int *nb_args )
{
	unsigned int i, pos;
	const char *arg_types = params;
	 ...
	 for (i = pos = 0; !is_ret_val( arg_types[i] ); i++)
    {
        ...
        {
        case 'j': /* int64 */
            //printf( "%x%08x", stack[pos+1], stack[pos] );
            FIXME( "%x%08x", stack[pos+1], stack[pos] );
            pos += 2;
            break;
        case 'k': /* int128 */
            //printf( "{%08x,%08x,%08x,%08x}", stack[pos], stack[pos+1], stack[pos+2], stack[pos+3] );
            FIXME( "{%08x,%08x,%08x,%08x}", stack[pos], stack[pos+1], stack[pos+2], stack[pos+3] );
            pos += 4;
            break;
        case 's': /* str */
            print_string_a( stack[pos++] );
            break;
        case 'w': /* wstr */
            print_string_w( stack[pos++] );
            break;
        case 'f': /* float */
            //printf( "%g", *(const float *)&stack[pos++] );
            FIXME( "%g", *(const float *)&stack[pos++] );
            break;
        case 'd': /* double */
            //printf( "%g", *(const double *)&stack[pos] );
            FIXME( "%g", *(const double *)&stack[pos] );
            pos += 2;
            break;
        case 'i': /* long */
        default:
            //printf( "%08x", stack[pos++] );
            FIXME( "%08x", stack[pos++] );
            break;
        }
        //if (!is_ret_val( arg_types[i+1] )) printf( "," );
        if (!is_ret_val( arg_types[i+1] )) FIXME( "," );
    }
    *nb_args = pos;
    if (arg_types[0] == 't')
    {
...
void WINAPI parse_exit( char* name, char* params,void *retaddr, LONGLONG retval )
{
    const char *arg_types = params;
    ...
    {
        //printf( " retval=%08x%08x ret=%08x\n",
          //     (UINT)(retval >> 32), (UINT)retval, (UINT)retaddr );
        FIXME( " retval=%08x%08x ret=%08x\n",
               (UINT)(retval >> 32), (UINT)retval, (UINT)retaddr );
    }
    else
    {
        //printf( " retval=%08x ret=%08x\n", (UINT)retval, (UINT)retaddr );
        FIXME( " retval=%08x ret=%08x\n", (UINT)retval, (UINT)retaddr );
    }
}

LONGLONG WINAPI entry_call_exit(void);
#define __i386__

#ifdef __i386__

__ASM_GLOBAL_FUNC( entry_call_exit,
                   "pushl %ebp\n\t"
                   "movl %esp,%ebp\n\t"
		   ...
		   "cld\n\t" 
		   "rep; movsl\n\t" 
		   "testl $0x80000000,-32(%ebp)\n\t" 
		   "jz 1f\n\t"
		   "popl %ecx\n\t" 
		   "testl $0x40000000,-32(%ebp)\n\t"   
		   "jz 1f\n\t"
		   "popl %edx\n" 
...
void set_hook(HANDLE module,const char* func_name,const char* param_list)
{
	void *mem;
	void* addr;
	char err[255];
	
	void *p_parse_entry = NULL;
	void *p_parse_exit;

	const char* t = param_list; 
	const char* h = NULL;
	char buf[128];
	int c = 0;
	...
	if(module == NULL)
	{
		FIXME("set_hook module is NULL\n");
		return;
	}
	mem = mmap(NULL, HOOK_CODE_SIZE, PROT_READ | PROT_WRITE |PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	if(mem==NULL)
	{
		printf("mmap failed.\n");
		return;
	}
	addr = wine_dlsym(module,func_name,err,255);
	if(addr == NULL)
	{
		printf("cannot find symbol %s in module=%p\n",func_name,module);
		return;
	}
	memset((char*)mem,0,HOOK_CODE_SIZE);
	memcpy((char*)mem,(char*)entry_call_exit,200);
	memcpy((char*)mem+NAME_OFFSET,(char*)func_name,strlen(func_name)+1);
	memcpy((char*)mem+ORIGFUNC_ADDR_OFFSET,(char*)&addr,sizeof(void*));
	p_parse_entry = parse_entry;
	memcpy((char*)mem+PARSE_ENTRY_ADDR_OFFSET,&p_parse_entry,sizeof(void*));
	p_parse_exit = parse_exit;
	memcpy((char*)mem+PARSE_EXIT_ADDR_OFFSET,&p_parse_exit,sizeof(void*));
	...
	for(;;)
	{
		...
		parsed:
			if(get_arg_type(buf)!=0)
			{
				char type = get_arg_type(buf);
				memcpy((char*)mem+PARAM_OFFSET+c,&type,sizeof(char));
				c++;
			}
	}
    ...
    memcpy((char*)mem+PARAM_OFFSET+c,&type,sizeof(char));
    type = '\0';
    memcpy((char*)mem+PARAM_OFFSET+c+1,&type,sizeof(char));

    func_jump(peekpeek,(void**)((char*)mem+ORIGFUNC_ADDR_OFFSET),mem);
}

void setup_api_hook(HANDLE handle,const char* func_name,const char* param_list)
{
	if(peekpeek != NULL) 
	{	
		set_hook(handle,func_name,param_list);
   	}
}
                             ...
                             const char *so_name, WINE_MODREF** pwm )
{
	...
	//peekpeek,begin
	if(strstr(debugstr_a(so_name) ,"user32.dll"))
	{
		int rv;

 	    	if(peekpeek == NULL)
		{		
	   		peekpeek = peekpeek_create();
		}
		if(peekpeek != NULL)
		{	//Here handle is the return value of wine_dlopen for user32.dll.so, see line 3011
			setup_api_hook(handle,"CreateWindowExW","long wstr wstr long long long long long long long long ptr");
			setup_api_hook(handle,"GetWindowTextA","long ptr long");		
			setup_api_hook(handle,"SetWindowTextA","long str");
		}
		else
			printf("create peekpeek failed\n");
	
		rv = peekpeek_install(peekpeek,0);
		if(rv!=0)
	   	{
		   printf("install peekpeek failed\n");
	   	}
